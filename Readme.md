...
如何忘记了id ，这里有个id的例子介绍：
<details> <summary> id的介绍</summary>
在Python中，id()函数返回对象的唯一标识符，它是一个整数，在对象的生命周期内保持不变。这个标识符通常对应于对象在内存中的地址。当我们使用id()函数时，可以清楚地看到不同对象的内存地址是不同的，而相同对象的内存地址是相同的。这对于理解Python的变量引用机制和对象传递方式非常有帮助。
</details>
...
这里记住的是类型是属于对象的，而不是变量。而对象有两种,"可更改"（mutable）与"不可更改"（immutable）对象。<span style="color:red">在python中，strings, tuples, 和numbers是不可更改的对象，而 list, dict, set 等则是可以修改的对象。(这就是这个问题的重点)</span>

当一个引用传递给函数的时候,函数自动复制一份引用,这个函数里的引用和外边的引用没有半毛关系了.所以第一个例子里函数把引用指向了一个不可变对象,当函数返回的时候,外面的引用没半毛感觉.而第二个例子就不一样了,函数内的引用指向的是**可变对象**,对它的操作就**和定位了指针地址一样**,在**内存里**进行**修改**.
